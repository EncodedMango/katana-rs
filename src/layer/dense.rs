use std::ops::Div;
use arrayfire::{af_print, Array, constant, dim4, FloatingPoint, HasAfEnum, matmul, MatProp, randn, tile};
use crate::Layer;
...
impl<T: HasAfEnum + FloatingPoint> DenseLayer<T> {
    pub fn new(n_inputs: u64, n_neurons: u64) -> Self {
        Self {weights: randn::<T>(dim4!(n_inputs, n_neurons)).div(constant(10, dim4!(n_inputs, n_neurons)).cast()), biases: Array::new_empty(dim4!(1, n_neurons))}
    }
}
